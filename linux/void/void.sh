#!/usr/bin/env bash
# Void Linux System Setup Script
# Fully configures a clean install of Void with all needed tools and configurations
...
LOG_FILE="/var/log/void_setup.log"
...
PACKAGES=(
  ...
  base-devel cmake ninja meson gettext git nmap
...
#------------------------------------------------------------
# update_system
#    Updates package repositories and upgrades the system using xbps.
#------------------------------------------------------------
update_system() {
  log_info "Updating package repositories and upgrading system..."
  if ! xbps-install -Syu; then
    handle_error "Failed to update/upgrade system." 1
  fi
...
#------------------------------------------------------------
# install_packages
#    Installs essential packages via xbps.
#------------------------------------------------------------
install_packages() {
...
#------------------------------------------------------------
# prompt_for_password
#    Prompts for and sets a password for user $USERNAME.
#------------------------------------------------------------
prompt_for_password() {
  local pass1 pass2
...
#------------------------------------------------------------
# create_user
#    Creates the user if not already present, sets a password,
#    adds the user to the wheel group, and installs/configures doas.
#------------------------------------------------------------
create_user() {
  ...
  # Add user to wheel group for admin privileges.
  ...
  if ! command -v doas &>/dev/null; then
    ...
    if ! xbps-install -Sy doas; then
      handle_error "Failed to install doas." 1
    fi
    log_info "doas installed successfully."
  fi

  # Configure doas for the wheel group in /etc/doas.conf.
  local doas_conf_file="/etc/doas.conf"
...
#------------------------------------------------------------
# configure_timezone
#    Installs chrony (if needed), enables its runit service, and starts it.
#------------------------------------------------------------
configure_timezone() {
  ...
  if command -v chronyd &>/dev/null; then
    ...
    if ! xbps-install -Sy chrony; then
      handle_error "Failed to install chrony." 1
    fi
  fi
  # Enable chronyd via runit.
  if [ ! -L /var/service/chronyd ]; then
    ln -s /etc/sv/chronyd /var/service || handle_error "Failed to enable chronyd service." 1
  fi
  if ! sv status chronyd &>/dev/null; then
    sv start chronyd || handle_error "Failed to start chronyd service." 1
  fi
...
#------------------------------------------------------------
# configure_ssh
#    Installs OpenSSH if needed, enables its runit service, and restarts it.
#------------------------------------------------------------
configure_ssh() {
  log_info "Configuring SSH service..."
  if ! command -v sshd &>/dev/null; then
    if ! xbps-install -Sy openssh; then
      handle_error "Failed to install openssh." 1
    fi
  fi
  # Enable sshd via runit.
  if [ ! -L /var/service/sshd ]; then
    ln -s /etc/sv/sshd /var/service || handle_error "Failed to enable sshd service." 1
  fi
  sv restart sshd || handle_error "Failed to restart sshd." 1
...
secure_ssh_config() {
  ...
  sv restart sshd || handle_error "Failed to restart sshd after hardening." 1
  log_info "SSH configuration hardened successfully."
}

#------------------------------------------------------------
# configure_firewall
#    Configures iptables with secure default policies and adds necessary rules.
#------------------------------------------------------------
configure_firewall() {
...
#------------------------------------------------------------
# configure_runit_firewall
#    Sets up a runit service to restore firewall rules at boot.
#------------------------------------------------------------
configure_runit_firewall() {
  log_info "Configuring runit service for firewall persistence..."
  local sv_dir="/etc/sv/firewall"
  if [ ! -d "$sv_dir" ]; then
    mkdir -p "$sv_dir" || handle_error "Failed to create $sv_dir." 1
  fi
  local run_file="$sv_dir/run"
  if [ -f "$run_file" ]; then
    log_info "Runit firewall service already configured. Skipping."
  else
    cat << 'EOF' > "$run_file"
#!/bin/sh
...
EOF
    chmod +x "$run_file" || handle_error "Failed to make $run_file executable." 1
  fi
  if [ ! -L /var/service/firewall ]; then
    ln -s "$sv_dir" /var/service/firewall || handle_error "Failed to enable firewall runit service." 1
  fi
  log_info "Firewall runit service configured successfully."
}

#------------------------------------------------------------
# configure_runit_services
#    Ensures that essential runit services (syslog and crond) are enabled.
#------------------------------------------------------------
configure_runit_services() {
  log_info "Ensuring runit services for syslog and crond are enabled..."
  if [ -d /etc/sv/syslog ] && [ ! -L /var/service/syslog ]; then
    ln -s /etc/sv/syslog /var/service || handle_error "Failed to enable syslog service." 1
    log_info "Syslog service enabled."
  else
    log_info "Syslog service already enabled or not installed."
  fi
  if [ -d /etc/sv/crond ] && [ ! -L /var/service/crond ]; then
    ln -s /etc/sv/crond /var/service || handle_error "Failed to enable crond service." 1
    log_info "Crond service enabled."
  else
    log_info "Crond service already enabled or not installed."
  fi
}
...
#------------------------------------------------------------
# setup_cron
#    Starts the crond service using runit.
#------------------------------------------------------------
setup_cron() {
  log_info "Starting crond service..."
  if command -v crond &>/dev/null || [ -d /etc/sv/crond ]; then
    sv start crond || handle_error "Failed to start crond service." 1
...
#------------------------------------------------------------
# dotfiles_load
#    Copies .bashrc and .profile from the dotfiles repository into the user and root home directories.
#------------------------------------------------------------
dotfiles_load() {
  log_info "Copying dotfiles (.bashrc and .profile) to user and root home directories..."
  local source_dir="/home/${USERNAME}/github/bash/linux/void/dotfiles"
...
#------------------------------------------------------------
# set_default_shell
#    Sets bash (/bin/bash) as the default shell for both $USERNAME and root.
#------------------------------------------------------------
set_default_shell() {
...
main() {
  ...
  dotfiles_load
  set_default_shell
  log_info "Void Linux system setup completed successfully."
}
