#!/usr/bin/env bash
################################################################################
# Ubuntu Automated Setup Script - Enhanced Edition
################################################################################
# Description:
#   This script automates the configuration of a fresh Ubuntu server system,
#   creating a secure, optimized, and personalized environment. Key features:
...
#       build dependencies, and various optional services (Caddy, Plex, VSCode CLI).
...
#   PACKAGES) as needed. All actions and errors are logged to /var/log/ubuntu_setup.log.
#
# Compatibility:
#   Tested on Ubuntu 24.10. Verify compatibility on other versions if needed.
#
...
# ------------------------------------------------------------------------------
LOG_FILE="/var/log/ubuntu_setup.log"      # Log file path
...
PACKAGES=(
  ...
  git ufw perl curl wget tcpdump rsync htop passwd bash-completion neofetch tig jq fonts-dejavu-core fonts-firacode
  ...
  gnome-keyring seahorse thunar dmenu i3 i3status feh alacritty picom
)
...
YELLOW='\033[38;2;235;203;139m'   # Warnings/labels
GREEN='\033[38;2;163;190;140m'    # Success/Info
BLUE='\033[38;2;94;129;172m'      # Debug/Highlights
CYAN='\033[38;2;136;192;208m'     # Headings/Accent
...
configure_ssh() {
    ...
    # Backup and update sshd_config with best practices
...
install_caddy() {
    ...
    apt install -y ubuntu-keyring apt-transport-https curl || handle_error "Failed to install prerequisites for Caddy."
...
install_vscode_cli() {
    ...
    log INFO "Visual Studio Code CLI installed successfully. Run './code tunnel --name ubuntu-server' to start the tunnel."
}
...
finalize_configuration() {
    ...
    if command -v snap &>/dev/null; then
        snap refresh || handle_error "Failed to refresh Snap packages."
    else
        log INFO "Snap not installed; skipping Snap refresh."
    fi
...
main() {
    log INFO "======================================"
    log INFO "Starting Ubuntu Automated Setup Script"
    log INFO "======================================"
    ...
    log INFO "Ubuntu system setup completed successfully."
    prompt_reboot
}
