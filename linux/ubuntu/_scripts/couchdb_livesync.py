...
VERSION: str = "1.0.1"  # Bumped version number
DEFAULT_USERNAME: str = os.environ.get("USER") or "user"
OPERATION_TIMEOUT: int = 120  # Extended for longer operations
...
class CouchDBConfig:
    ...
    def get_effective_hostname(self) -> str:
        """Get the effective hostname to use (never empty)."""
        if self.domain and self.domain != DEFAULT_DOMAIN:
            return self.domain
        return "localhost"
...
async def async_prompt(message: str, default: str = "") -> str:
    """Async-compatible wrapper for Prompt.ask with default value."""
...
async def run_command_async(
    cmd: List[str], check_sudo: bool = False, retry: int = RETRY_COUNT
) -> Tuple[int, str, str]:
    """Run a shell command asynchronously with retry logic.
    ...
    while attempt < max(1, retry):
        attempt += 1
        if attempt > 1:
            print_warning(f"Retrying command (attempt {attempt}/{retry})...")
        ...
        try:
            ...
            app_logger.debug(f"Running command: {' '.join(cmd)}")
            ...
            # Log command results
            app_logger.command(cmd, proc.returncode, stdout, stderr)

            # If command succeeded, return the results
            if proc.returncode == 0 or attempt >= retry:
                return proc.returncode, stdout, stderr

            # Save this error for potential retry
            last_error = stderr
    ...
    return 1, "", f"Command failed after {retry} attempts: {last_error}"
...
async def check_service_status(service_name: str) -> Tuple[bool, str]:
    ...
    if service_name.startswith("snap."):
        ...
        if "couchdb" in service_name:
            ...
            if returncode == 0 or "200" in stdout:
                app_logger.info(
...
async def validate_domain(domain: str) -> Tuple[bool, str]:
    """Validate domain format and ensure it's not empty.
...
async def configure_couchdb(config: CouchDBConfig) -> bool:
    ...
    with Progress(
        ...
        await asyncio.sleep(5)  # Increased from 3 to 5 seconds for better reliability
...
async def run_livesync_init(config: CouchDBConfig = None) -> bool:
    """Run the LiveSync initialization script with more detailed error capture and proper hostname.
    ...
    # Determine the hostname to use - NEVER allow empty hostname
    hostname = config.get_effective_hostname()
...
async def troubleshoot_async(config: CouchDBConfig, status: InstallStatus) -> None:
    ...
    for component in failed_components:
        if component == "Snapd":
            ...
            table.add_row(
                ...
                "2. Try running script manually:\n   curl -s https://raw.githubusercontent.com/vrtmrz/obsidian-livesync/main/utils/couchdb/couchdb-init.sh | bash -s -- -h your-hostname\n"
    ...
    if proceed:
        ...
        # Check hostname
        returncode, stdout, stderr = await run_command_async(["hostname"])
        hostname = stdout if returncode == 0 else "Unknown"
        ...
        diagnostics_table.add_row("Hostname", hostname)
        diagnostics_table.add_row("Disk Space", disk_space)
        diagnostics_table.add_row("CouchDB Response", couchdb_response)
        diagnostics_table.add_row("Configured Domain", config.domain)
        diagnostics_table.add_row("Effective Hostname", config.get_effective_hostname())
...
async def install_manager_async() -> None:
    ...
    if not config.domain or config.domain == DEFAULT_DOMAIN:
        while True:
            ...
            # Validate domain
            is_valid, error_message = await validate_domain(config.domain)
            if is_valid:
                break
            else:
                print_error(f"Invalid domain: {error_message}")
                print_warning(
                    "Please enter a valid domain or leave empty to use the default."
                )
    ...
    while True:
        ...
        if choice.lower() in ("q", "quit", "exit"):
            ...
            while True:
                ...
                if not new_domain:
                    # User didn't enter anything, keep current domain
                    break

                # Validate domain
                is_valid, error_message = await validate_domain(new_domain)
                if is_valid:
                    ...
                    print_error(f"Invalid domain: {error_message}")
                    print_warning(
                        "Please enter a valid domain or leave empty to keep the current domain."
                    )
