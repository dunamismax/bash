...
def print_header(message: str) -> None:
    ...
    for line in ascii_art.splitlines():
        print(f"{NordColors.BG_DARK}{NordColors.BOLD}{line.center(term_width)}{NordColors.RESET}")
    print(f"{NordColors.BG_DARK}{NordColors.FROST_2}{' ' * term_width}{NordColors.RESET}\n")
...
class ProgressBar:
    ...
    def _display(self) -> None:
        ...
        eta_str = format_time((self.total - self.current) * elapsed / self.current) if (elapsed > 0 and self.current > 0) else "N/A"
        sys.stdout.write(
...
class Spinner:
    ...
    def stop(self, success: bool = True, message: str = None) -> None:
        if self.running:
            ...
            if self.thread:
                self.thread.join()
            status_symbol = f"{NordColors.SUCCESS}✓" if success else f"{NordColors.ERROR}✗"
...
def install_packages(packages: List[str]) -> bool:
    ...
    success, _, _ = run_command(f"{APT_CMD} update", error_message="Failed to update package lists", check=False, spinner_text="Updating package lists")
    if not success:
    ...
    for package in packages:
        success, _, _ = run_command(f"{APT_CMD} install -y {package}", error_message=f"Failed to install {package}", check=False, capture_output=True)
        if success:
    ...
    failed_packages = []
    for package in packages:
        success, _, _ = run_command(f"dpkg -s {package}", check=False, capture_output=True)
        if not success:
...
def enable_zfs_services() -> bool:
    ...
    for service in ZFS_SERVICES:
        success, _, _ = run_command(f"systemctl enable {service}", error_message=f"Failed to enable {service}", check=False)
        if success:
...
def is_pool_imported(pool_name: str) -> bool:
    success, _, _ = run_command(f"zpool list {pool_name}", error_message=f"Pool {pool_name} is not imported", check=False, spinner_text=f"Checking if pool '{pool_name}' is imported")
    return success
...
def import_zfs_pool(pool_name: str, force: bool = False) -> bool:
    ...
    success, stdout, stderr = run_command(f"zpool import {force_flag} {pool_name}", error_message=f"Failed to import ZFS pool '{pool_name}'", check=False, spinner_text=f"Importing ZFS pool '{pool_name}'")
    if success:
...
def configure_zfs_pool(pool_name: str, mount_point: str, cache_file: str) -> bool:
    ...
    success, _, stderr = run_command(f"zfs set mountpoint={mount_point} {pool_name}", error_message=f"Failed to set mountpoint for '{pool_name}'", check=False, spinner_text=f"Setting mountpoint to '{mount_point}'")
    if not success:
    ...
    success, _, stderr = run_command(f"zpool set cachefile={cache_file} {pool_name}", error_message=f"Failed to set cachefile for '{pool_name}'", check=False, spinner_text=f"Setting cachefile to '{cache_file}'")
    if not success:
...
def mount_zfs_datasets() -> bool:
    ...
    success, stdout, stderr = run_command("zfs mount -a", error_message="Failed to mount ZFS datasets", check=False, spinner_text="Mounting all ZFS datasets")
    if success:
...
def verify_mount(pool_name: str, mount_point: str) -> bool:
    ...
    success, stdout, _ = run_command("zfs list -o name,mountpoint -H", error_message="Failed to list ZFS filesystems", check=False, spinner_text="Verifying mount status")
    if not success:
...
def show_zfs_status(pool_name: str) -> None:
    ...
    success, stdout, _ = run_command(f"zpool status {pool_name}", error_message=f"Failed to get status for pool '{pool_name}'", check=False)
    if success and stdout:
        print(f"{NordColors.FROST_3}{stdout}{NordColors.RESET}")
    else:
        print_warning(f"Could not get pool status for '{pool_name}'")
    success, stdout, _ = run_command(f"zpool get all {pool_name}", error_message=f"Failed to get properties for pool '{pool_name}'", check=False)
    if success and stdout:
...
def interactive_setup() -> Tuple[str, str, str, bool]:
    ...
    if available_pools:
        ...
        pool_name = input(f"{NordColors.FROST_1}Enter pool name [{DEFAULT_POOL_NAME}]: {NordColors.RESET}").strip() or DEFAULT_POOL_NAME
    else:
    ...
    mount_point = input(f"{NordColors.FROST_1}Enter mount point [{default_mount}]: {NordColors.RESET}").strip() or default_mount
    cache_file = input(f"{NordColors.FROST_1}Enter cache file path [{DEFAULT_CACHE_FILE}]: {NordColors.RESET}").strip() or DEFAULT_CACHE_FILE
    force_input = input(f"{NordColors.FROST_1}Force import if needed? (y/N): {NordColors.RESET}")
    ...
    print(f"  {NordColors.FROST_3}Force import: {NordColors.RESET}{'Yes' if force else 'No'}")
    confirm = input(f"{NordColors.FROST_1}Proceed with this configuration? (Y/n): {NordColors.RESET}")
...
def execute_zfs_setup(args: Any) -> bool:
    ...
    try:
        ...
        if not args.skip_install:
            ...
            print_step("Skipping ZFS package installation (--skip-install)", current_step, steps_total)
        current_step += 1
...
def get_zfs_list() -> Optional[List[Dict[str, str]]]:
    output = run_command_simple("zfs list -o name,used,available,refer,mountpoint -t all -H")
    if not output:
        return None
    datasets = []
...
@cli.command()
@click.option("--pool-name", default=DEFAULT_POOL_NAME, help="Name of the ZFS pool to import")
@click.option("--mount-point", default=None, help="Mount point for the ZFS pool (default: /media/{pool_name})")
@click.option("--cache-file", default=DEFAULT_CACHE_FILE, help="Path to the ZFS cache file")
...
def setup(pool_name: str, mount_point: Optional[str], cache_file: str, force: bool, skip_install: bool, verbose: bool, interactive: bool) -> None:
    ...
    if success:
        ...
        print(f"{NordColors.FROST_3}Your ZFS pool is now configured and imported.{NordColors.RESET}")
        print(f"{NordColors.FROST_3}Access your data at: {NordColors.RESET}{mount_point}")
        print(f"{NordColors.FROST_3}Helpful ZFS commands:{NordColors.RESET}")
        print(f"  {NordColors.FROST_1}zfs list{NordColors.RESET}              - List ZFS filesystems")
        print(f"  {NordColors.FROST_1}zpool status {pool_name}{NordColors.RESET}  - Show pool status")
        print(f"  {NordColors.FROST_1}zfs get all {pool_name}{NordColors.RESET}   - Show all properties")
