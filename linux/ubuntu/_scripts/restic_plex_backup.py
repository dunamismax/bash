...
def check_plex_installed() -> bool:
    ...
    if not Path("/var/lib/plexmediaserver").exists():
        print_error(
            "Plex Media Server installation not found. Expected path: /var/lib/plexmediaserver"
        )
        return False
    return True
...
def perform_backup() -> bool:
    ...
    if total_size == 0:
        ...
        return False
    console.print(
        f"Preparing to backup {file_count} files ({_format_size(total_size)})",
        style="#D8DEE9",
    )
...
def main(non_interactive: bool, retention: str, debug: bool) -> None:
    ...
    if not (
        ...
        and check_environment()
        and check_plex_installed()
    ):
        sys.exit(1)
    ...
    try:
        ...
        console.print(
            f"[bold #8FBCBB]Plex Media Server backup completed successfully.[/bold #8FBCBB]"
        )
        console.print(
