...
def print_section(title: str) -> None:
    ...
    border = "═" * TERM_WIDTH
    console.print(f"\n[bold {NordColors.NORD8}]{border}[/bold {NordColors.NORD8}]")
    console.print(f"[bold {NordColors.NORD8}]  {title.center(TERM_WIDTH - 4)}[/bold {NordColors.NORD8}]")
    console.print(f"[bold {NordColors.NORD8}]{border}[/bold {NordColors.NORD8}]\n")
...
def print_success(message: str) -> None:
    ...
    console.print(f"[bold {NordColors.NORD14}]✓ {message}[/bold {NordColors.NORD14}]")

def print_warning(message: str) -> None:
    """Print a warning message."""
    console.print(f"[bold {NordColors.NORD13}]⚠ {message}[/bold {NordColors.NORD13}]")

def print_error(message: str) -> None:
    """Print an error message."""
    console.print(f"[bold {NordColors.NORD11}]✗ {message}[/bold {NordColors.NORD11}]")

def print_step(text: str, step_num: int = None, total_steps: int = None) -> None:
    """Print a step description, optionally with step numbers."""
    step_info = f"[{step_num}/{total_steps}] " if (step_num is not None and total_steps is not None) else ""
...
# Logging Setup
# ----------------------------------------------------------------
def setup_logging(log_file: str = DEFAULT_LOG_FILE, log_level: int = logging.INFO) -> None:
...
def cleanup() -> None:
    ...
    # Remove temporary VS Code package if it exists
    try:
...
class ProgressBar:
    ...
    def _display(self) -> None:
        ...
        console.print(
            f"\r[#88C0D0]{self.desc}:[/#88C0D0] |[#5E81AC]{bar}[/#5E81AC]| [#D8DEE9]{percent:5.1f}%[/#D8DEE9] [ETA: {eta_str}]",
            end="",
        )
        if self.completed:
            elapsed_str = format_time(elapsed)
            console.print(
                f"\r[#88C0D0]{self.desc}:[/#88C0D0] |[#5E81AC]{bar}[/#5E81AC]| [#A3BE8C]100.0%[/#A3BE8C] [Completed in: {elapsed_str}]"
            )
...
class Spinner:
    ...
    def _spin(self) -> None:
        while self.spinning:
            ...
            with self._lock:
                console.print(
                    f"\r[#5E81AC]{self.spinner_chars[self.current]}[/#5E81AC] [#88C0D0]{self.message}[/#88C0D0] [[dim]elapsed: {time_str}[/dim]]",
                    end="",
                )
    ...
    def stop(self, success: bool = True, message: str = None) -> None:
        with self._lock:
            ...
            completion_message = message if message else ("Completed" if success else "Failed")
            if success:
                console.print(f"[#A3BE8C]✓[/#A3BE8C] [#88C0D0]{self.message}[/#88C0D0] [#A3BE8C]{completion_message}[/#A3BE8C] in {time_str}")
            else:
                console.print(f"[#BF616A]✗[/#BF616A] [#88C0D0]{self.message}[/#88C0D0] [#BF616A]{completion_message}[/#BF616A] after {time_str}")
    def __enter__(self):
        self.start()
        return self
...
def run_command_simple(command: Union[str, List[str]], verbose: bool = False) -> Optional[str]:
...
def configure_zfs_pool(pool_name: str, mount_point: str, cache_file: str, verbose: bool = False) -> bool:
    ...
    if not success:
        ...
        if stderr:
            print_error(f"Error details: {stderr}")
        print_warning("Pool was imported but cachefile was not set. Automatic mounting on boot may not work.")
        return False
...
def verify_mount(pool_name: str, mount_point: str, verbose: bool = False) -> bool:
    ...
    if pool_found and correct_mount:
        ...
        print_warning(f"ZFS pool '{pool_name}' is mounted at '{actual_mount}' (expected: '{mount_point}').")
        return False
    else:
...
def show_zfs_status(pool_name: str, verbose: bool = False) -> None:
    ...
    if success and stdout:
        important_props = ["size", "capacity", "health", "fragmentation", "free", "allocated"]
        filtered_output = []
        for line in stdout.splitlines():
...
# ----------------------------------------------------------------
def get_zpool_status(verbose: bool = False) -> Optional[Dict[str, List[Dict[str, Any]]]]:
    ...
    for line in output.splitlines():
        ...
        if current_pool:
            ...
            if line and not any(line.startswith(prefix) for prefix in ("errors:", "config:", "capacity:")):
                parts = line.split()
                if len(parts) >= 2 and parts[1] in ["ONLINE", "DEGRADED", "OFFLINE", "FAULTED", "REMOVED", "UNAVAIL"]:
                    current_pool["vdevs"].append({"type": "disk", "path": parts[0], "state": parts[1]})
                    continue
...
def get_zfs_list(verbose: bool = False) -> Optional[List[Dict[str, str]]]:
    ...
    output = run_command_simple("zfs list -o name,used,available,refer,mountpoint -t all -H", verbose)
    if not output:
        return None
    datasets = []
...
def set_autoexpand_property(pool_name: str, verbose: bool = False) -> bool:
    ...
    if autoexpand_value != "on":
        ...
        if run_command_simple(f"zpool set autoexpand=on {pool_name}", verbose) is not None:
...
def verify_pool_resize(pool_name: str, verbose: bool = False) -> bool:
    ...
    print_info(f"Initial allocatable pool size: {bytes_to_human_readable(initial_size)}")
...
def expand_zpool(pool_name: str, device_path: str, verbose: bool = False) -> bool:
    ...
    if run_command_simple(f"zpool online -e {pool_name} {device_path}", verbose) is None:
...
def interactive_setup() -> Tuple[str, str, str, bool]:
    ...
    if available_pools:
        ...
        pool_name = console.input(f"[bold #88C0D0]Enter pool name [{DEFAULT_POOL_NAME}]: [/bold #88C0D0]").strip() or DEFAULT_POOL_NAME
    else:
    ...
    mount_point = console.input(f"[bold #88C0D0]Enter mount point [{default_mount}]: [/bold #88C0D0]").strip() or default_mount
    cache_file = console.input(f"[bold #88C0D0]Enter cache file path [{DEFAULT_CACHE_FILE}]: [/bold #88C0D0]").strip() or DEFAULT_CACHE_FILE
    force_input = console.input(f"[bold #88C0D0]Force import if needed? (y/N): [/bold #88C0D0]")
    ...
    console.print(f"  Force import: [bold #88C0D0]{'Yes' if force else 'No'}[/bold #88C0D0]")
    confirm = console.input(f"[bold #88C0D0]Proceed with this configuration? (Y/n): [/bold #88C0D0]")
...
def execute_zfs_setup(args) -> bool:
    ...
    try:
        ...
        if not args.skip_install:
            ...
            print_step("Skipping ZFS package installation (--skip-install)", current_step, total_steps)
...
""",
    ...
    setup_parser.add_argument("--pool-name", default=DEFAULT_POOL_NAME, help="Name of the ZFS pool to import")
    setup_parser.add_argument("--mount-point", default=None, help="Mount point for the ZFS pool (default: /media/{pool_name})")
    setup_parser.add_argument("--cache-file", default=DEFAULT_CACHE_FILE, help="Path to the ZFS cache file")
    setup_parser.add_argument("--log-file", default=DEFAULT_LOG_FILE, help="Path to the log file")
    setup_parser.add_argument("--force", action="store_true", help="Force import of the ZFS pool")
    ...
    setup_parser.add_argument("--interactive", action="store_true", help="Run interactive setup")

    # Expand command
    expand_parser = subparsers.add_parser("expand", help="Expand ZFS pools to use full device size")
    expand_parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose output")

    parser.add_argument("--version", action="version", version=f"Unified ZFS Management Script v{VERSION}")
...
def main() -> None:
    ...
    try:
        ...
        if not args.command:
            ...
            print_info("Use 'setup' or 'expand' command. See --help for more information.")
            sys.exit(1)
        ...
        if args.command == "setup":
            ...
            if not run_command_simple("modprobe zfs"):
                print_warning("ZFS kernel module could not be loaded. You may need to install ZFS packages first.")
            if args.interactive:
                try:
                    args.pool_name, args.mount_point, args.cache_file, args.force = interactive_setup()
            ...
            print_info(f"Mount point: {args.mount_point or DEFAULT_MOUNT_POINT.format(pool_name=args.pool_name)}")
            ...
            if success:
                ...
                print_info(f"Access your data at: {args.mount_point or DEFAULT_MOUNT_POINT.format(pool_name=args.pool_name)}")
                print_info("Helpful ZFS commands:")
                console.print(f"  [bold #88C0D0]zfs list[/bold #88C0D0]              - List ZFS filesystems")
                console.print(f"  [bold #88C0D0]zpool status {args.pool_name}[/bold #88C0D0]  - Show pool status")
                console.print(f"  [bold #88C0D0]zfs get all {args.pool_name}[/bold #88C0D0]   - Show all properties")
            sys.exit(0 if success else 1)
        elif args.command == "expand":
            print_header("ZFS Pool Expansion")
            print_info(f"Started at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            if not check_root_privileges():
                sys.exit(1)
            ...
            for pool_name, result in expansion_results.items():
                status_text = "[bold #A3BE8C]Successful[/bold #A3BE8C]" if result else "[bold #BF616A]Failed[/bold #BF616A]"
            ...
            overall_color = "#A3BE8C" if overall == "Successful" else "#BF616A"
            console.print(f"Overall validation: [bold {overall_color}]{overall}[/bold {overall_color}]")
            print_info(f"Completed at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
