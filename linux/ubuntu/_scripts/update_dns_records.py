...
using the Cloudflare API via the standard library. It provides comprehensive logging,
error handling, and graceful signal handling.

Usage:
  sudo ./update_dns_records.py [--non-interactive]

Notes:
  - This script must be run with root privileges.
  - Requires environment variables CF_API_TOKEN and CF_ZONE_ID (e.g., set in /etc/environment).
...
TERM_WIDTH = 80  # Adjust based on terminal size
...
def check_dependencies() -> None:
    ...
    try:
        import rich  # already used above
        import pyfiglet
