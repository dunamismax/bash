...
def start_virtual_machines(vms: List[Dict[str, str]]) -> bool:
    ...
    to_start: List[Dict[str, str]] = [
        vm for vm in vms if vm["state"].lower() != "running"
    ]
    if not to_start:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if (
                        ...
                        print_warning(
                            f"{name} failed to start due to live display conflict; retrying in 5 seconds..."
                        )
                        time.sleep(5)
                    else:
...
def configure_user_groups() -> bool:
    ...
    try:
        ...
        print_success(
            f"User {sudo_user} added to {LIBVIRT_USER_GROUP}. Please log out/in for changes to take effect."
        )
        return True
    except Exception as e:
