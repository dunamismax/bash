...
def calculate_checksum() -> bool:
    ...
    if not path:
        ...
        return False
    algorithm_options = [(str(i + 1), algo.upper()) for i, algo in enumerate(CHECKSUM_ALGORITHMS)]
    ...
    try:
        ...
        elapsed = time.time() - start_time
        result_panel = Panel(f"[bold {NordColors.NORD8}]{checksum}[/]", title=f"{algorithm.upper()} Checksum", border_style=f"bold {NordColors.NORD14}")
...
def disk_usage() -> bool:
    ...
    summary_table.add_row("Large files (>" + format_size(threshold) + ")", str(len(large_files)))
    ...
    if category_sizes:
        ...
        category_table.add_column("Percentage", style=f"{NordColors.NORD7}", justify="right")
        for category, size in sorted(category_sizes.items(), key=lambda x: x[1], reverse=True):
            percentage = (size / total_size * 100) if total_size > 0 else 0
            category_table.add_row(category.capitalize(), format_size(size), f"{percentage:.1f}%")
    ...
    if large_files:
        ...
        large_file_table.add_column("Size", style=f"{NordColors.NORD11}", justify="right")
...
def batch_operation_menu() -> None:
    ...
    options = [("1", "Batch Copy"), ("2", "Batch Move"), ("3", "Batch Delete"), ("0", "Back to Main Menu")]
