...
def signal_handler(signum: int, frame: Any) -> None:
    console.print(
        f"[{NordColors.WARNING}]Script interrupted by {signal.Signals(signum).name}. Cleaning up...[/]"
    )
    sys.exit(128 + signum)
...
def check_root() -> None:
    if os.geteuid() != 0:
        console.print(
            f"[{NordColors.ERROR}]{NordColors.BOLD}Error: This script must be run with root privileges.[/]",
            style=NordColors.ERROR,
        )
        sys.exit(1)
    ...
    while retries <= MAX_RETRIES:
        try:
            ...
            if "timeout" in error_msg.lower() or "connection" in error_msg.lower():
                ...
                console.print(
                    f"[{NordColors.WARNING}]Transient error; retrying in {delay} seconds (attempt {retries}).[/]"
                )
                time.sleep(delay)
            else:
                console.print(
                    f"[{NordColors.ERROR}]Restic command failed: {error_msg}[/]"
                )
                raise
...
def get_latest_snapshot(repo: str) -> Optional[str]:
    ...
    try:
        ...
        if not snapshots:
            console.print(
                f"[{NordColors.WARNING}]No snapshots found in repository: {repo}[/]"
            )
            return None
        ...
        console.print(
            f"[{NordColors.ERROR}]Error retrieving snapshots for {repo}: {e}[/]"
        )
        return None
...
def scan_for_repos() -> Dict[int, Tuple[str, str]]:
    ...
    try:
        ...
        console.print(
            f"[{NordColors.ERROR}]Error scanning B2 bucket: {e.stderr or str(e)}[/]"
        )
    return repos
...
def restore_repo(repo: str, target: Path) -> bool:
    ...
    if not snap_id:
        console.print(
            f"[{NordColors.ERROR}]Skipping restore for {repo} – no snapshot found.[/]"
        )
        return False
    target.mkdir(parents=True, exist_ok=True)
    console.print(
        f"[{NordColors.INFO}]Restoring snapshot {snap_id} from {repo} into {target}...[/]"
    )
    try:
        run_restic(
        ...
        if not any(target.iterdir()):
            console.print(
                f"[{NordColors.ERROR}]Restore failed: {target} is empty after restore.[/]"
            )
            return False
        console.print(
            f"[{NordColors.SUCCESS}]Successfully restored {repo} into {target}.[/]"
        )
        return True
    except Exception as e:
...
def main(non_interactive: bool, repo: str) -> None:
    ...
    if non_interactive and repo:
        ...
        if not available_repos:
            console.print(
                f"[{NordColors.ERROR}]No restic repositories found in bucket {B2_BUCKET}.[/]"
            )
            sys.exit(1)
        ...
        try:
            ...
            console.print(
                f"[{NordColors.ERROR}]Invalid input. Please enter valid numbers separated by spaces.[/]"
            )
            sys.exit(1)
        selected_repos = {
        ...
        if not selected_repos:
            console.print(
                f"[{NordColors.ERROR}]No valid repositories selected. Exiting.[/]"
            )
            sys.exit(1)
    ...
    for _, (repo_name, repo_path) in selected_repos.items():
        ...
        console.print(
            f"[{NordColors.INFO}]Restoring repository '{repo_name}' from {repo_path} into {target_dir}...[/]"
        )
    ...
    for repo_name, success in results.items():
        status = (
            f"[{NordColors.SUCCESS}]SUCCESS[/]"
            if success
            else f"[{NordColors.ERROR}]FAILED[/]"
        )
