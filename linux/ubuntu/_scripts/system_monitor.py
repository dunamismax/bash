...
def display_cpu_results(results: dict) -> None:
    ...
    if "error" in results:
        ...
        return
    console.print(
        f"CPU Cores (Physical): [bold {NordColors.CPU}]{results['cores']}[/bold {NordColors.CPU}]"
    )
    console.print(
        f"CPU Threads (Logical): [bold {NordColors.CPU}]{results['threads']}[/bold {NordColors.CPU}]"
    )
    console.print(
        f"CPU Frequency: [bold {NordColors.CPU}]{results['frequency_current']:.2f} MHz[/bold {NordColors.CPU}]"
    )
    console.print(
        f"CPU Usage: [bold {NordColors.CPU}]{results['usage']:.2f}%[/bold {NordColors.CPU}]"
    )
    console.print(
        f"Benchmark Duration: [bold {NordColors.CPU}]{results['elapsed_time']:.2f} sec[/bold {NordColors.CPU}]"
    )
    console.print(
        f"[bold {NordColors.SUCCESS}]✓ Prime Numbers per Second: {results['primes_per_sec']:.2f}[/bold {NordColors.SUCCESS}]"
    )
...
def display_gpu_results(results: dict) -> None:
    ...
    console.print(
        f"Benchmark Duration: [bold {NordColors.GPU}]{results['elapsed_time']:.2f} sec[/bold {NordColors.GPU}]"
    )
    console.print(
        f"[bold {NordColors.SUCCESS}]✓ Matrix Multiplications per Second: {results['iterations_per_sec']:.2f}[/bold {NordColors.SUCCESS}]"
    )
    if "name" in gpu_info:
        console.print(
            f"GPU Name: [bold {NordColors.GPU}]{gpu_info['name']}[/bold {NordColors.GPU}]"
        )
        console.print(
            f"GPU Load: [bold {NordColors.GPU}]{gpu_info['load']:.2f}%[/bold {NordColors.GPU}]"
        )
        console.print(
            f"GPU Memory Utilization: [bold {NordColors.GPU}]{gpu_info['memory_util']:.2f}%[/bold {NordColors.GPU}]"
        )
        console.print(
            f"GPU Temperature: [bold {NordColors.GPU}]{gpu_info['temperature']:.2f}°C[/bold {NordColors.GPU}]"
        )
...
def monitor_menu() -> None:
    ...
    while True:
        ...
        console.print(
            f"1. Refresh Rate: [bold {NordColors.LABEL}]{refresh_rate} seconds[/bold {NordColors.LABEL}]"
        )
        console.print(
            f"2. Duration: [bold {NordColors.LABEL}]{duration if duration > 0 else 'Unlimited'} seconds[/bold {NordColors.LABEL}]"
        )
        console.print(
            f"3. Export Format: [bold {NordColors.LABEL}]{export_format if export_format else 'None'}[/bold {NordColors.LABEL}]"
        )
        console.print(
            f"4. Export Interval: [bold {NordColors.LABEL}]{export_interval} minutes[/bold {NordColors.LABEL}]"
        )
        console.print(
            f"5. Output File: [bold {NordColors.LABEL}]{output_file if output_file else 'Auto-generated'}[/bold {NordColors.LABEL}]"
        )
        console.print(
            f"6. Sort Processes By: [bold {NordColors.LABEL}]{sort_by.upper()}[/bold {NordColors.LABEL}]"
        )
        console.print(
            f"7. [bold {NordColors.SUCCESS}]Start Monitor[/bold {NordColors.SUCCESS}]"
        )
...
def benchmark_menu() -> None:
    ...
    while True:
        ...
        print_section("Current Settings")
        console.print(
            f"Benchmark Duration: [bold {NordColors.LABEL}]{duration} seconds[/bold {NordColors.LABEL}]"
        )
        ...
        try:
            ...
            if choice == "1":
                ...
                with console.status(
                    f"[bold {NordColors.CPU}]Running benchmarks for {duration} seconds...",
                    spinner="dots",
                ):
...
def display_system_info() -> None:
    ...
    console.print(
        f"Hostname: [bold {NordColors.LABEL}]{hostname}[/bold {NordColors.LABEL}]"
    )
    console.print(
        f"Time: [bold {NordColors.LABEL}]{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/bold {NordColors.LABEL}]"
    )
    console.print(
        f"Uptime: [bold {NordColors.LABEL}]{get_system_uptime()}[/bold {NordColors.LABEL}]"
    )
    cpu_info = get_cpu_info()
    console.print(
        f"CPU: [bold {NordColors.CPU}]{cpu_info['cores']} cores / {cpu_info['threads']} threads[/bold {NordColors.CPU}]"
    )
    mem = psutil.virtual_memory()
    console.print(
        f"Memory: [bold {NordColors.MEM}]{mem.total / (1024**3):.2f} GB total[/bold {NordColors.MEM}]"
    )
        ...
        try:
            ...
            if choice == "1":
                ...
                for i, usage in enumerate(cpu_usage):
                    console.print(
                        f"Core {i}: [bold {NordColors.CPU}]{usage}%[/bold {NordColors.CPU}]"
                    )
                avg = sum(cpu_usage) / len(cpu_usage)
                console.print(
                    f"Average: [bold {NordColors.CPU}]{avg:.1f}%[/bold {NordColors.CPU}]"
                )
                load = get_load_average()
                console.print(
                    f"Load Average: [bold {NordColors.LOAD}]{load[0]:.2f}, {load[1]:.2f}, {load[2]:.2f}[/bold {NordColors.LOAD}]"
                )
                temp = get_cpu_temperature()
                if temp:
                    console.print(
                        f"Temperature: [bold {NordColors.TEMP}]{temp:.1f}°C[/bold {NordColors.TEMP}]"
                    )
            elif choice == "4":
