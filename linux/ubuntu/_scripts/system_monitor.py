...
def print_success(text: str) -> None:
    ...
    console.print(f"[bold {NORD_COLORS['success']}]✓ {text}[/bold {NORD_COLORS['success']}]")

def print_warning(text: str) -> None:
    """Print a warning message."""
    console.print(f"[bold {NORD_COLORS['warning']}]⚠ {text}[/bold {NORD_COLORS['warning']}]")

def print_error(text: str) -> None:
    """Print an error message."""
    console.print(f"[bold {NORD_COLORS['error']}]✗ {text}[/bold {NORD_COLORS['error']}]")
...
def cpu_benchmark(benchmark_duration: int = DEFAULT_BENCHMARK_DURATION) -> dict:
    ...
    with console.status(f"[bold {NORD_COLORS['gpu']}]Running CPU benchmark for {benchmark_duration} seconds...", spinner="dots"):
...
def gpu_matrix_benchmark(benchmark_duration: int) -> dict:
    ...
    while time.time() < end_time:
        ...
        iterations += 1
    elapsed = time.time() - start_time
    return {'iterations_per_sec': iterations / elapsed if elapsed > 0 else 0,
            'elapsed_time': elapsed, 'gpu_object': gpu}
...
def gpu_benchmark(benchmark_duration: int = DEFAULT_BENCHMARK_DURATION) -> dict:
    ...
    with console.status(f"[bold {NORD_COLORS['gpu']}]Running GPU benchmark for {benchmark_duration} seconds...", spinner="dots"):
...
def display_cpu_results(results: dict) -> None:
    ...
    console.print(f"CPU Cores (Physical): [bold {NORD_COLORS['cpu']}]{results['cores']}[/bold {NORD_COLORS['cpu']}]")
    console.print(f"CPU Threads (Logical): [bold {NORD_COLORS['cpu']}]{results['threads']}[/bold {NORD_COLORS['cpu']}]")
    console.print(f"CPU Frequency (Current): [bold {NORD_COLORS['cpu']}]{results['frequency_current']:.2f} MHz[/bold {NORD_COLORS['cpu']}]")
    console.print(f"CPU Usage during Benchmark: [bold {NORD_COLORS['cpu']}]{results['usage']:.2f}%[/bold {NORD_COLORS['cpu']}]")
    console.print(f"Benchmark Duration: [bold {NORD_COLORS['cpu']}]{results['elapsed_time']:.2f} seconds[/bold {NORD_COLORS['cpu']}]")
    console.print(f"[bold {NORD_COLORS['success']}]✓ Prime Numbers per Second: {results['primes_per_sec']:.2f}[/bold {NORD_COLORS['success']}]")
    console.print("\n[bold " + NORD_COLORS['cpu'] + "]Benchmark Details:[/bold " + NORD_COLORS['cpu'] + "]")
...
def display_gpu_results(results: dict) -> None:
    ...
    if 'error' in results:
        ...
        console.print(f"[bold {NORD_COLORS['error']}]{results['error']}[/bold {NORD_COLORS['error']}]")
        console.print("\n[bold " + NORD_COLORS['warning'] + "]Troubleshooting Tips:[/bold " + NORD_COLORS['warning'] + "]")
        ...
        console.print(f"GPU Name: [bold {NORD_COLORS['gpu']}]{results['name']}[/bold {NORD_COLORS['gpu']}]")
        console.print(f"Benchmark Duration: [bold {NORD_COLORS['gpu']}]{results['elapsed_time']:.2f} seconds[/bold {NORD_COLORS['gpu']}]")
        console.print(f"[bold {NORD_COLORS['success']}]✓ Matrix Multiplications per Second: {results['iterations_per_sec']:.2f}[/bold {NORD_COLORS['success']}]")
        console.print(f"GPU Load during Benchmark: [bold {NORD_COLORS['gpu']}]{results['load']:.2f}%[/bold {NORD_COLORS['gpu']}]")
        console.print(f"GPU Memory Utilization: [bold {NORD_COLORS['gpu']}]{results['memory_util']:.2f}%[/bold {NORD_COLORS['gpu']}]")
        console.print(f"GPU Temperature: [bold {NORD_COLORS['gpu']}]{results['temperature']:.2f}°C[/bold {NORD_COLORS['gpu']}]")
        console.print("\n[bold " + NORD_COLORS['gpu'] + "]Benchmark Details:[/bold " + NORD_COLORS['gpu'] + "]")
...
class DiskMonitor:
    ...
    def update(self) -> None:
        ...
        try:
            ...
            console.print(f"[bold {NORD_COLORS['error']}]Error updating disk info: {e}[/bold {NORD_COLORS['error']}]")
...
class NetworkMonitor:
    ...
    def update(self) -> None:
        ...
        try:
            ...
            console.print(f"[bold {NORD_COLORS['error']}]Error updating network info: {e}[/bold {NORD_COLORS['error']}]")
...
class UnifiedMonitor:
    ...
    def build_dashboard(self, sort_by: str) -> Layout:
        ...
        metrics_panel = Panel("\n".join(metrics), title="System Metrics", border_style=f"{NORD_COLORS['cpu']}")
        ...
        proc_panel = Panel("\n".join(proc_lines), title="Top Processes", border_style=f"{NORD_COLORS['proc']}")
        body = Layout()
        body.split_row(
        ...
        footer_text = f"[{NORD_COLORS['label']}]Press Ctrl+C to exit.[/{NORD_COLORS['label']}]"
    ...
    def export_data(self, export_format: str, output_file: Optional[str] = None) -> None:
        ...
        console.print(f"[bold {NORD_COLORS['success']}]Data exported to {output_file}[/bold {NORD_COLORS['success']}]")
...
def menu() -> None:
    ...
    while True:
        ...
        console.print(f"[bold {NORD_COLORS['section']}]Select a benchmark to run:[/bold {NORD_COLORS['section']}]")
        console.print(f"[bold {NORD_COLORS['section']}]1.[/bold {NORD_COLORS['section']}] CPU Benchmark")
        console.print(f"[bold {NORD_COLORS['section']}]2.[/bold {NORD_COLORS['section']}] GPU Benchmark")
        console.print(f"[bold {NORD_COLORS['section']}]3.[/bold {NORD_COLORS['section']}] CPU and GPU Benchmarks")
        console.print(f"[bold {NORD_COLORS['section']}]4.[/bold {NORD_COLORS['section']}] Exit")
        try:
            choice = click.prompt(f"[bold {NORD_COLORS['section']}]Enter your choice [1-4][/bold {NORD_COLORS['section']}]", type=click.IntRange(1, 4))
        except Exception as e:
        ...
        if choice == 1:
            ...
            console.print(f"[bold {NORD_COLORS['section']}]Exiting Benchmark Tool...[/bold {NORD_COLORS['section']}]")
            break
        else:
...
def monitor(refresh: float, duration: float, export: Optional[str],
    ...
    setup_logging()
    if os.geteuid() != 0:
        console.print(f"[bold {NORD_COLORS['error']}]This script must be run as root.[/bold {NORD_COLORS['error']}]")
        sys.exit(1)
    ...
    try:
        ...
        console.print(f"[bold {NORD_COLORS['error']}]Unexpected error: {e}[/bold {NORD_COLORS['error']}]")
        sys.exit(1)
