...
----------------------------------------------------------
Description:
  A robust unified restore solution that retrieves the latest snapshots from two restic
  repositories stored on Backblaze B2 and restores them locally. The two repositories
  are:
    1. VM Backup   - Contains libvirt virtual machine configurations and disk images.
    2. Plex Backup - Contains Plex Media Server configuration and application data.
...
# Restic repository strings for Backblaze B2 (only VM and Plex backups)
...
# Global restore status for reporting (only VM and Plex)
RESTORE_STATUS = {
...
def restore_repo(repo: str, password: str, restore_target: str, task_name: str) -> bool:
    ...
    try:
        ...
        if "repository is already locked" in err_output:
            logging.warning("Restore failed due to lock. Retrying after force unlock...")
...
def verify_restore(restore_path, start_time=None):
    ...
    try:
        ...
        verification["verification_duration"] = format_duration(time.time() - start_time)
        return verification
    except Exception as e:
...
def print_repository_info():
    """
    Display information about the VM and Plex repositories before restore.
    """
    print_section("Repository Information")
    repos = [("VM", B2_REPO_VM), ("Plex", B2_REPO_PLEX)]
...
def print_status_report():
    ...
    for task, data in RESTORE_STATUS.items():
        ...
        if not DISABLE_COLORS:
            ...
            logging.info(f"{icons.get(status, '?')} {task_desc}: {status.upper()} - {msg}")
        if status == "success" and stats:
            logging.info(f"    • Files Restored: {stats.get('files_restored', 'unknown')}")
            ...
            if stats.get("verified", False):
                ...
                logging.info(f"    • Verification: {stats.get('verification_note', 'Failed')}")
...
def main():
    ...
    logging.info("=" * 80)
    logging.info(f"UNIFIED RESTORE COMPLETED WITH {summary} AT {now} (took {elapsed:.1f} seconds)")
