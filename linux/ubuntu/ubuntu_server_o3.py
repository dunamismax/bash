#!/usr/bin/env python3
"""
Ubuntu Server Setup & Hardening Utility
----------------------------------------

This script automates the initialization and hardening of an Ubuntu server.
It performs preflight checks, system updates, package installations,
user environment setup, security hardening, service installations,
...
# Example package download settings (for illustration)
...
PACKAGES: List[str] = [
    ...
    "foot", "foot-themes", "tree", "ncdu", "neofetch", "build-essential",
    "cmake", "ninja-build", "meson", "gettext", "git", "pkg-config",
    "openssh-server", "ufw", "curl", "wget", "rsync", "sudo", "bash-completion",
    "python3", "python3-dev", "python3-pip", "python3-venv", "python3-rich",
    "python3-pyfiglet", "libssl-dev", "libffi-dev", "zlib1g-dev", "libreadline-dev",
    "libbz2-dev", "tk-dev", "xz-utils", "libncurses-dev", "libgdbm-dev",
    "libnss3-dev", "liblzma-dev", "libxml2-dev", "libxmlsec1-dev", "ca-certificates",
    "software-properties-common", "apt-transport-https", "gnupg", "lsb-release",
    "clang", "llvm", "netcat-openbsd", "lsof", "unzip", "zip", "net-tools",
    "nmap", "iftop", "iperf3", "tcpdump", "lynis", "traceroute", "mtr",
    "iotop", "glances", "golang-go", "gdb", "cargo", "fail2ban", "rkhunter",
    "chkrootkit", "postgresql-client", "mysql-client", "ruby", "rustc", "jq",
    "yq", "certbot", "p7zip-full", "qemu-system", "libvirt-clients",
    "libvirt-daemon-system", "virt-manager", "qemu-user-static", "nala",
]
...
# ----------------------------------------------------------------
def run_with_progress(desc: str, func, *args, task_name: Optional[str] = None, **kwargs) -> Any:
    """
...
def signal_handler(signum: int, frame: Optional[Any]) -> None:
    sig_name = getattr(signal, "Signals", lambda s: f"signal {s}")(signum).name if hasattr(signal, "Signals") else f"signal {signum}"
...
class Utils:
    ...
    @staticmethod
    def ensure_directory(path: str, owner: Optional[str] = None, mode: int = 0o755) -> bool:
        try:
...
class SystemUpdater:
    ...
    def fix_package_issues(self) -> bool:
        ...
        try:
            dpkg_status = Utils.run_command(["dpkg", "--configure", "-a"], check=False, capture_output=True)
            if dpkg_status.returncode != 0:
                logger.warning(f"dpkg issues: {dpkg_status.stderr}")
                Utils.run_command(["dpkg", "--configure", "-a"])
            held = Utils.run_command(["apt-mark", "showhold"], check=False, capture_output=True, text=True)
            if held.stdout.strip():
                for pkg in held.stdout.strip().split("\n"):
                    if pkg.strip():
                        try:
                            Utils.run_command(["apt-mark", "unhold", pkg.strip()], check=False)
                        except Exception as e:
            ...
            check = Utils.run_command(["apt-get", "check"], check=False, capture_output=True, text=True)
            if check.returncode != 0:
                logger.warning(f"apt-get check issues: {check.stderr}")
                Utils.run_command(["apt", "--fix-missing", "update"], check=False)
                Utils.run_command(["apt", "--fix-broken", "install", "-y"], check=False)
                final = Utils.run_command(["apt-get", "check"], check=False, capture_output=True, text=True)
    ...
    def update_system(self, full_upgrade: bool = False) -> bool:
        ...
        try:
            ...
            upgrade_cmd = ["nala", "full-upgrade", "-y"] if full_upgrade else ["nala", "upgrade", "-y"]
            try:
...
class UserEnvironment:
    ...
    def setup_repos(self) -> bool:
        ...
        for repo in repos:
            ...
            if os.path.isdir(os.path.join(repo_dir, ".git")):
                ...
                try:
                    Utils.run_command(["git", "clone", f"https://github.com/dunamismax/{repo}.git", repo_dir], check=False)
                except Exception:
    ...
    def copy_shell_configs(self) -> bool:
        ...
        src_dir = os.path.join(USER_HOME, "github", "bash", "linux", "ubuntu", "dotfiles")
            ...
            for d in dest_dirs:
                ...
                if copy_needed:
                    try:
                        ...
                        owner = f"{USERNAME}:{USERNAME}" if d == USER_HOME else "root:root"
        ...
        try:
            for item in os.listdir(src_dir):
                ...
                if os.path.isdir(src_path):
                    ...
                    Utils.run_command(["rsync", "-a", "--update", f"{src_path}/", f"{dest_path}/"])
                    Utils.run_command(["chown", "-R", f"{USERNAME}:{USERNAME}", dest_path])
            return success
        except Exception as e:
        ...
        try:
            ...
            if "/bin/bash" not in shells:
                with open("/etc/shells", "a") as f:
                    f.write("/bin/bash\n")
            current_shell = subprocess.check_output(["getent", "passwd", USERNAME], text=True).strip().split(":")[-1]
...
class SecurityHardener:
    ...
    def configure_firewall(self) -> bool:
        ...
        try:
            ...
            pass
        for cmd in ([ufw_cmd, "default", "deny", "incoming"], [ufw_cmd, "default", "allow", "outgoing"]):
            try:
        ...
        for port in ALLOWED_PORTS:
            try:
                ...
                pass
        try:
            status = Utils.run_command([ufw_cmd, "status"], capture_output=True, text=True)
    ...
    def configure_fail2ban(self) -> bool:
        ...
        try:
            ...
            status = Utils.run_command(["systemctl", "is-active", "fail2ban"], capture_output=True, text=True, check=False)
    ...
    def configure_apparmor(self) -> bool:
        ...
        try:
            ...
            status = Utils.run_command(["systemctl", "is-active", "apparmor"], capture_output=True, text=True, check=False)
...
class ServiceInstaller:
    ...
    def docker_config(self) -> bool:
        ...
        if not Utils.command_exists("docker"):
            try:
                ...
                Utils.run_command(["curl", "-fsSL", "https://get.docker.com", "-o", script_path])
    ...
    def install_configure_caddy(self) -> bool:
        ...
        if not caddy_installed:
            try:
                Utils.run_command(["nala", "install", "-y", "debian-keyring", "debian-archive-keyring", "apt-transport-https", "curl"])
                Utils.run_command("curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg", shell=True)
                Utils.run_command("curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list", shell=True)
                Utils.run_command(["nala", "update"])
        ...
        try:
            ...
            caddy_src = os.path.join(USER_HOME, "github", "bash", "linux", "ubuntu", "dotfiles", "Caddyfile")
            ...
            status = Utils.run_command(["systemctl", "is-active", "caddy"], capture_output=True, text=True, check=False)
            return status.stdout.strip() == "active"
        except Exception:
    ...
    def install_enable_tailscale(self) -> bool:
        ...
        if not Utils.command_exists("tailscale"):
            try:
                Utils.run_command(["sh", "-c", "curl -fsSL https://tailscale.com/install.sh | sh"])
            except Exception:
                return False
        try:
            Utils.run_command(["systemctl", "enable", "tailscaled"])
            Utils.run_command(["systemctl", "start", "tailscaled"])
            status = Utils.run_command(["systemctl", "is-active", "tailscaled"], capture_output=True, text=True, check=False)
            return status.stdout.strip() == "active"
        except Exception:
    ...
    def deploy_user_scripts(self) -> bool:
        ...
        try:
            ...
            Utils.run_command(["find", tgt, "-type", "f", "-exec", "chmod", "755", "{}", ";"])
    ...
    def configure_unattended_upgrades(self) -> bool:
        ...
        try:
            if not SystemUpdater().install_packages(["unattended-upgrades", "apt-listchanges"]):
                return False
            ...
            status = Utils.run_command(["systemctl", "is-active", "unattended-upgrades"], capture_output=True, text=True, check=False)
            return status.stdout.strip() == "active"
        except Exception:
            return False
    ...
    def update_ssl_certificates(self) -> bool:
        ...
        if not Utils.command_exists("certbot"):
            if not SystemUpdater().install_packages(["certbot"]):
                return False
        try:
            output = Utils.run_command(["certbot", "renew", "--dry-run"], capture_output=True, text=True).stdout
...
class SystemTuner:
    ...
    def home_permissions(self) -> bool:
        ...
        try:
            ...
            Utils.run_command(["find", USER_HOME, "-type", "d", "-exec", "chmod", "g+s", "{}", ";"])
            if Utils.command_exists("setfacl"):
                Utils.run_command(["setfacl", "-R", "-d", "-m", f"u:{USERNAME}:rwX,g:{USERNAME}:r-X,o::---", USER_HOME])
            return True
        except Exception:
            return False
...
class FinalChecker:
    ...
    def system_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pass
        try:
            df_lines = subprocess.check_output(["df", "-h", "/"], text=True).strip().splitlines()
            if len(df_lines) >= 2:
                data = df_lines[1].split()
                health["disk"] = {"total": data[1], "used": data[2], "available": data[3], "percent_used": data[4]}
        except Exception:
            pass
        try:
            free_lines = subprocess.check_output(["free", "-h"], text=True).strip().splitlines()
            for line in free_lines:
                if line.startswith("Mem:"):
                    parts = line.split()
                    health["memory"] = {"total": parts[1], "used": parts[2], "free": parts[3]}
        except Exception:
            pass
        try:
            with open("/proc/loadavg") as f:
                load = f.read().split()[:3]
            health["load"] = {"1min": float(load[0]), "5min": float(load[1]), "15min": float(load[2])}
        except Exception:
            pass
        try:
            dmesg_output = subprocess.check_output(["dmesg", "--level=err,crit,alert,emerg"], text=True, stderr=subprocess.DEVNULL).strip()
            health["kernel_errors"] = bool(dmesg_output)
        except Exception:
            pass
        try:
            updates = subprocess.check_output(["nala", "list", "--upgradable"], text=True, stderr=subprocess.DEVNULL).strip().splitlines()
    ...
    def verify_firewall_rules(self) -> bool:
        ...
        for port in ALLOWED_PORTS:
            try:
                result = subprocess.run(["nc", "-z", "-w3", "127.0.0.1", port], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def final_checks(self) -> bool:
        ...
        try:
            ...
            if float(load_avg[1]) > cpu_count:
                all_passed = False
            services = ["ssh", "ufw", "fail2ban", "caddy", "docker", "tailscaled", "unattended-upgrades"]
            for svc in services:
                status = subprocess.run(["systemctl", "is-active", svc], text=True, capture_output=True, check=False)
                if status.stdout.strip() != "active" and svc in ["ssh", "ufw"]:
                    all_passed = False
            try:
                unattended_output = subprocess.check_output(["unattended-upgrade", "--dry-run", "--debug"], text=True, stderr=subprocess.STDOUT)
                if any("Packages that will be upgraded:" in line and "0 upgrades" not in line for line in unattended_output.splitlines()):
                    all_passed = False
            except Exception:
                pass
            return all_passed
    ...
    def cleanup_system(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                installed = subprocess.check_output(["dpkg", "--list", "linux-image-*", "linux-headers-*"], text=True).strip().splitlines()
                old_kernels = [line.split()[1] for line in installed if line.startswith("ii") and line.split()[1] not in (f"linux-image-{current_kernel}", f"linux-headers-{current_kernel}") and "generic" in line.split()[1]]
            ...
            for tmp in ["/tmp", "/var/tmp"]:
                try:
                    Utils.run_command(["find", tmp, "-type", "f", "-atime", "+7", "-not", "-path", "*/\\.*", "-delete"])
                except Exception:
                    pass
            try:
                log_files = subprocess.check_output(["find", "/var/log", "-type", "f", "-size", "+50M"], text=True).strip().splitlines()
                for lf in log_files:
...
class UbuntuServerSetup:
    ...
    def run(self) -> int:
        ...
        try:
            run_with_progress("Running Pre-flight Checks...", self.preflight.check_root, task_name="preflight")
            if not self.preflight.check_network():
                logger.error("Network check failed.")
                SETUP_STATUS["preflight"] = {"status": "failed", "message": "Network check failed"}
                sys.exit(1)
        ...
        run_with_progress("Running apt --fix-broken install", fix_broken, task_name="fix_broken")

        # Phase 2: Installing Nala
        print_section("Phase 2: Installing Nala")
        try:
            if not run_with_progress("Installing Nala...", self.services.install_nala, task_name="nala_install"):
        ...
        try:
            if not run_with_progress("Updating system...", self.updater.update_system, task_name="system_update"):
            ...
            self.success = False
        try:
            if not run_with_progress("Installing packages...", self.updater.install_packages, task_name="packages_install"):
            ...
            self.success = False
        try:
            if not run_with_progress("Configuring timezone...", self.updater.configure_timezone):
        ...
        try:
            if not run_with_progress("Setting up user repositories...", self.user_env.setup_repos, task_name="user_env"):
            ...
            self.success = False
        try:
            if not run_with_progress("Copying shell configs...", self.user_env.copy_shell_configs):
            ...
            self.success = False
        try:
            if not run_with_progress("Copying config folders...", self.user_env.copy_config_folders):
        ...
        try:
            if not run_with_progress("Configuring SSH...", self.security.configure_ssh, task_name="security"):
        ...
        try:
            ...
            self.success = False
        try:
            if not run_with_progress("Configuring firewall...", self.security.configure_firewall):
            ...
            self.success = False
        try:
            if not run_with_progress("Configuring Fail2ban...", self.security.configure_fail2ban):
            ...
            self.success = False
        try:
            if not run_with_progress("Configuring AppArmor...", self.security.configure_apparmor):
        ...
        try:
            if not run_with_progress("Installing Fastfetch...", self.services.install_fastfetch, task_name="services"):
        ...
        try:
            ...
            self.success = False
        try:
            if not run_with_progress("Installing Tailscale...", self.services.install_enable_tailscale):
            ...
            self.success = False
        try:
            if not run_with_progress("Configuring unattended upgrades...", self.services.configure_unattended_upgrades):
            ...
            self.success = False
        try:
            if not run_with_progress("Installing Caddy...", self.services.install_configure_caddy):
            ...
            self.success = False
        try:
            if not run_with_progress("Deploying user scripts...", self.services.deploy_user_scripts):
        ...
        try:
            if not run_with_progress("Configuring periodic maintenance...", self.maintenance.configure_periodic, task_name="maintenance"):
            ...
            self.success = False
        try:
            if not run_with_progress("Backing up configurations...", self.maintenance.backup_configs):
            ...
            self.success = False
        try:
            if not run_with_progress("Updating SSL certificates...", self.maintenance.update_ssl_certificates):
        ...
        try:
            if not run_with_progress("Applying system tuning...", self.tuner.tune_system, task_name="tuning"):
            ...
            self.success = False
        try:
            if not run_with_progress("Securing home directory...", self.tuner.home_permissions):
        ...
        SETUP_STATUS["final"] = {"status": "in_progress", "message": "Running final checks..."}
        try:
        ...
        if self.success and final_result:
            SETUP_STATUS["final"] = {"status": "success", "message": f"Completed in {int(minutes)}m {int(seconds)}s."}
        else:
            SETUP_STATUS["final"] = {"status": "warning", "message": f"Completed with warnings in {int(minutes)}m {int(seconds)}s."}
        try:
            status_report()
        except Exception as e:
