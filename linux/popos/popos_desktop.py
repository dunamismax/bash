...
class PopOSDesktopSetup:
    ...
    async def copy_shell_configs_async(self) -> bool:
        ...
        if not source_dir.is_dir():
            self.logger.error(f"Pop!_OS-specific dotfiles not found in {source_dir}.")
            return False
    ...
    async def copy_config_folders_async(self) -> bool:
        ...
        if not src.is_dir():
            self.logger.error(
                f"Pop!_OS-specific dotfiles directory not found in {src}."
            )
            return False
    ...
    async def configure_ssh_async(self) -> bool:
        try:
            result = await run_command_async(
                ["dpkg", "-s", "openssh-server"], check=False, capture_output=True
            )
            if result.returncode != 0:
    ...
    async def install_plex_async(self) -> bool:
        try:
            result = await run_command_async(
                ["dpkg", "-s", "plexmediaserver"], check=False, capture_output=True
            )
            if result.returncode == 0:
    ...
    async def install_fastfetch_async(self) -> bool:
        try:
            result = await run_command_async(
                ["dpkg", "-s", "fastfetch"], check=False, capture_output=True
            )
            if result.returncode == 0:
    ...
    async def install_configure_zfs_async(self) -> bool:
        ...
        try:
            result = await run_command_async(
                ["zpool", "list", pool], check=False, capture_output=True
            )
            if result.returncode == 0:
    ...
    async def install_flatpak_and_apps_async(self) -> Tuple[List[str], List[str]]:
        try:
            ...
            self.logger.info("Flatpak and its plugin installed using nala.")
        except subprocess.CalledProcessError as e:
    ...
    async def configure_wayland_async(self) -> bool:
        ...
        try:
            current = (
                await loop.run_in_executor(None, etc_env.read_text)
                if etc_env.is_file()
                else ""
            )
            vars_current = {}
...
async def main_async() -> None:
    ...
    try:
        ...
        if not await setup.phase_preflight():
            sys.exit(1)
        if not await setup.phase_install_nala():
            sys.exit(1)
