...
# Fully configures a clean install of Debian with all needed tools
# and configurations for hardening, security, and development.
#
# Enhancements/Improvements:
#  - Checks distribution to ensure the script is being run on Debian.
#  - Sets environment variables to prevent interactive prompts.
#  - Optional Fail2ban install & config for better SSH brute-force protection.
#  - Optional Unattended Upgrades install & config for automatic security updates.
#  - A few minor best-practice improvements in ordering and checks.
#
# Usage:
#   1) Run as root:  sudo ./setup_debian.sh
#   2) Toggle any optional functions near the bottom in main().
#
# Author:    <Your Name or GitHub handle here>
...
#--------------------------------------------------------------------------
# Environment Variables (Optional - helps avoid interactive prompts)
#--------------------------------------------------------------------------
...
secure_ssh_config() {
  ...
  # Ensure empty passwords are disallowed
...
configure_nftables_firewall() {
  ...
  # Write new nftables configuration
...
#--------------------------------------------------------------------------
# home_permissions
#    Ensures that the user’s home directory has the correct ownership & perms.
#--------------------------------------------------------------------------
home_permissions() {
...
#--------------------------------------------------------------------------
# dotfiles_load
#    Copies .bashrc and .profile from the dotfiles repo into user & root homedirs.
#--------------------------------------------------------------------------
dotfiles_load() {
...
#    Installs Nala on Debian using the Volian Scar repo installation script.
#--------------------------------------------------------------------------
install_and_configure_nala() {
...
#--------------------------------------------------------------------------
# configure_unattended_upgrades (OPTIONAL)
#    Installs and sets up unattended-upgrades for automatic security updates.
#--------------------------------------------------------------------------
configure_unattended_upgrades() {
  if ! dpkg -l unattended-upgrades &>/dev/null; then
    ...
    log_info "unattended-upgrades is already installed. Skipping installation."
  fi

  log_info "Configuring unattended-upgrades for automatic security updates..."
  dpkg-reconfigure -f noninteractive unattended-upgrades || log_warn "dpkg-reconfigure might have encountered an issue."

  # Optionally configure /etc/apt/apt.conf.d/50unattended-upgrades to your needs
  # Example snippet to enable all updates (not just security):
