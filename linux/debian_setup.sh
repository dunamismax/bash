...
#         - Deploys development and build dependencies along with additional
...
#
# Author : sawyer (adapted for Debian)
# License: MIT
################################################################################
...
# -------------------------------------------------------------------------------
# Essential Package List (CLI-only environment, Debian)
# -------------------------------------------------------------------------------
PACKAGES=(
...
install_packages() {
    ...
    if [ "${#to_install[@]}" -gt 0 ]; then
        ...
        apt-get install -y "${to_install[@]}" || handle_error "Failed to install essential packages."
    else
...
install_build_dependencies() {
    ...
    local deps=(
        gcc g++ make cmake git curl wget vim tmux unzip zip
        ca-certificates lsb-release gnupg jq pkg-config
        libssl-dev libbz2-dev libffi-dev zlib1g-dev libreadline-dev
        libsqlite3-dev tk-dev libncurses-dev libgdbm-dev liblzma-dev gdb llvm
    )
    apt-get install -y "${deps[@]}" || handle_error "Failed to install build dependencies."
    ...
    apt-get install -y golang || handle_error "Failed to install Go."
...
install_and_mount_zfs() {
    ...
    if ! dpkg -s zfsutils-linux &>/dev/null; then
        ...
        apt-get install -y zfsutils-linux || handle_error "Failed to install ZFS packages."
    else
...
configure_automatic_updates() {
    ...
    cat <<EOF >/etc/systemd/system/debian-auto-update.service
[Unit]
Description=Debian Automatic Update Service
...
EOF
    ...
    cat <<EOF >/etc/systemd/system/debian-auto-update.timer
[Unit]
Description=Daily Debian Automatic Update Timer
...
EOF
    ...
    systemctl daemon-reload
    systemctl enable --now debian-auto-update.timer || warn "Could not enable automatic update timer."
...
cleanup_system() {
    ...
    apt-get clean || warn "Failed to clean package cache."
