...
install_and_enable_plex() {
    ...
    local plex_group="plex"
    local media_dir="/var/db/plexdata"
    local config_dir="/usr/local/etc/plexmediaserver"
    local rc_enabled=""
    ...
    # Use LC_ALL=C to prevent encoding issues
    export LC_ALL=C
    ...
    if pkg info "${pkg_name}" >/dev/null 2>&1; then
        ...
        # Check rc.conf status safely
        rc_enabled=$(sysrc -n "${plex_service}_enable" 2>/dev/null || echo "NO")
        ...
        # Update package repository with retry logic
        while [ ${retry_count} -lt ${max_retries} ]; do
            log INFO "Updating package repository (attempt $((retry_count + 1))/${max_retries})..."
        ...
        if [ ${retry_count} -eq ${max_retries} ]; then
            log ERROR "Failed to update package repository after ${max_retries} attempts."
            return 1
        fi

        # Reset retry counter for installation
        retry_count=0
        
        # Install Plex with retry logic
        while [ ${retry_count} -lt ${max_retries} ]; do
            log INFO "Installing Plex Media Server (attempt $((retry_count + 1))/${max_retries})..."
            if pkg install -y "${pkg_name}"; then
                break
            fi
            retry_count=$((retry_count + 1))
        ...
        if [ ${retry_count} -eq ${max_retries} ]; then
            log ERROR "Failed to install Plex Media Server after ${max_retries} attempts."
            return 1
        fi
    fi

    # Create and configure directories
    for dir in "${media_dir}" "${config_dir}"; do
        if [ ! -d "${dir}" ]; then
    ...
    if [ "${rc_enabled}" != "YES" ]; then
        ...
        if ! sysrc "${plex_service}_support_path=${media_dir}" >/dev/null 2>&1; then
            log WARNING "Failed to set Plex support path in rc.conf."
        fi
    fi

    # Stop service if running (clean start)
    service "${plex_service}" status >/dev/null 2>&1 && {
        log INFO "Stopping existing Plex Media Server service..."
        service "${plex_service}" stop >/dev/null 2>&1
        sleep 2
    }
    ...
    while [ ${retry_count} -lt ${TIMEOUT} ]; do
        if service "${plex_service}" status >/dev/null 2>&1; then
            ...
            if pgrep -f "${plex_service}" >/dev/null 2>&1; then
                log INFO "Plex Media Server is now running."
                break
            fi
        fi
        retry_count=$((retry_count + 1))
    ...
    if [ ${retry_count} -eq ${TIMEOUT} ]; then
        log ERROR "Plex Media Server failed to start within ${TIMEOUT} seconds."
        return 1
    fi
