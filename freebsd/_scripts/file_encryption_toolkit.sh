...
#              management toolkit for FreeBSD. This interactive tool offers a
...
calculate_checksum() {
    ...
    case "$type" in
        1)
            md5 "$file" || handle_error "MD5 checksum calculation failed."
            ;;
        2)
            sha256 "$file" || handle_error "SHA256 checksum calculation failed."
            ;;
        *)
