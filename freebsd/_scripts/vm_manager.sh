...
#              to list, create, start, stop, delete, and monitor QEMU/libvirt
#              virtual machines on FreeBSD. It uses virt‑install and virsh to guide
#              you through VM creation (including options for RAM, vCPUs, disk size,
...
# GLOBAL VARIABLES
# ------------------------------------------------------------------------------
# FreeBSD libvirt typically uses /usr/local/var/lib/libvirt for images and ISOs.
VM_IMAGE_DIR="/usr/local/var/lib/libvirt/images"
ISO_DIR="/usr/local/var/lib/libvirt/boot"
mkdir -p "$ISO_DIR"
